...
window.__JCEF_READY__ = false;
window.__JCEF_QUEUE__ = [];

// Kotlin 调用这个
window.__JCEF_ON_READY__ = function (payload) {
    window.__JCEF_READY__ = true;

    // 注入数据
    if (payload?.options) {
        window.svgoOptions = payload.options;
    }

    if (payload?.svg) {
        window.pendingSvg = payload.svg;
    }
...
// JS 主动通知 Kotlin：我准备好了
function notifyKotlinReady() {
    if (window.JBCefAppReady) {
        window.JBCefAppReady("ready");
    }
}

// 页面真正启动逻辑（替代你原来的 bootstrap入口）
function startApp() {
    const options = window.svgoOptions || [];
